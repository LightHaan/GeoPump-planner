...
| air_t_delta_temp_at_reference_depth_c | degC | Paired `deltaT20New`; the derived warming-rate increment at the reference depth. |
| air_t_gradient_c_per_m | degC/m | Published per-metre warming rate represented by `deltaT20New / reference_depth_m`; its scientific basis is the borehole-to-surface temperature difference over borehole measurement depth. |
...
| surface_t_delta_temp_at_reference_depth_c | degC | Paired `deltaT20`; the derived warming-rate increment at the reference depth. |
| surface_t_gradient_c_per_m | degC/m | Published per-metre warming rate represented by `deltaT20 / reference_depth_m`; its scientific basis is the borehole-to-surface temperature difference over borehole measurement depth. |
